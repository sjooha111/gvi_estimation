...
Diko, A. ReViT: Enhancing vision transformers feature diversity with attention residual connections. Pattern Recognit (2024)
"""
import math
...
class TransformerBasicHead(nn.Module):
    ...
    def __init__(
        ...
        self.projection = nn.Linear(dim_in+2, 384, bias=True)
        self.dropout=nn.Dropout(0.2)
        self.L1head=nn.Linear(384,1)
    ...
    def forward(self, x,gx):
        ...
        x=self.sigmoid(self.projection(x))
        x=self.dropout(x)
        x=self.L1head(x) 
        return x
...
class ReViT(nn.Module):
    ...
    def __init__(self, cfg): # cfg : 딥러닝 설정 configuration 든 객체
        super().__init__()
        # Prepare input.
        in_chans = 19
